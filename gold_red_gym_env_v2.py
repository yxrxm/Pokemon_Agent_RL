...
# 여기서 부터 해보자---------------------------------------------------------------------------------
    def update_seen_coords(self):
        # if not in battle
        if self.read_m(0xD116) == 0:  # Gemini가 알려줬음, 확인해봤지만, 맞는지 잘 모르겠음.
            x_pos, y_pos, map_n = self.get_game_coords()
    ...
    def read_hp_fraction(self):
        hp_sum = sum([
            self.read_hp(add)
            for add in [0xDA4C, 0xDA7C, 0xDAAC, 0xDADC, 0xDB0C, 0xDB3C] # Hp 저장되어있는 주소의 시작점(두 바이트에 거쳐 저장되어있는데, 제미나이가 시작점의 주소만 적으면 된대)
        ])
        max_hp_sum = sum([
            self.read_hp(add)
            for add in [0xDA4E, 0xDA7E, 0xDAAE, 0xDADE, 0xDB0E, 0xDB3E] # Max Hp 저장되어있는 주소의 시작점(두 바이트에 거쳐 저장되어있는데, 제미나이가 시작점의 주소만 적으면 된대)
        ])
