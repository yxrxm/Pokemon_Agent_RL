...
# from HybridGoldEnv import HybridGoldEnv

# BC_CKPT = "metamon_offline/bc/data/bc_policy.pt"

# def make_env(rank, env_conf, seed=0):
#     def _init():
#         env = HybridGoldEnv(env_conf, bc_ckpt_path=BC_CKPT, bc_prob=1.0)
...
# 1. 설정 및 경로
# ==========================================
BC_CKPT = "metamon_offline/bc/data/bc_policy_v11.pt"  # 11차원 BC 모델
RED_WEIGHTS = "./poke_26214400/policy.pth"            # Red 버전 전이 가중치
LOG_DIR = "./runs"

def make_env(rank, env_conf, seed=0):
    def _init():
        # HybridGoldEnv를 사용하여 필드=PPO, 전투=BC 분리
        env = HybridGoldEnv(env_conf, bc_ckpt_path=BC_CKPT, bc_prob=1.0)
        env.reset(seed=(seed + rank))
        return env
    set_random_seed(seed)
...
# ==========================================
# 2. 스마트 학습 로직 (Freeze / Unfreeze / Transfer)
# ==========================================

def freeze_layers(model, freeze_modules=['features_extractor']):
    print(f"\n🥶 [전이 학습] '{freeze_modules}' 레이어를 동결합니다.")
    for name, param in model.policy.named_parameters():
        if any(module in name for module in freeze_modules):
...
class UnfreezeCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.is_frozen and self.num_timesteps >= self.unfreeze_step:
            print(f"\n🔥 [해제] {self.num_timesteps} 스텝 도달! 전 레이어 학습을 시작합니다.")
            for param in self.model.policy.parameters():
                param.requires_grad = True
            self.model.learning_rate = 0.0001 # 미세 조정을 위해 학습률 감소
            self.is_frozen = False
        return True
...
if __name__ == '__main__':
    sess_id = f"auto_{str(uuid.uuid4())[:4]}"
    sess_path = Path(f'session_{sess_id}')
    sess_path.mkdir(exist_ok=True)

    env_config = {
        'headless': True,           # 학습 속도를 위해 True 권장
        'save_final_state': True, 
        'action_freq': 24, 
        'init_state': './init.state', 
        'max_steps': 2048 * 10, 
        'print_rewards': False,     # 로그 가독성을 위해 False
        'gb_path': './PokeGold.gbc', 
        'session_path': sess_path,
        'fast_video': True,
        'save_video': False
    ...
    if os.path.exists(RED_WEIGHTS):
        model = transfer_weights(model, RED_WEIGHTS)
        # 2. 시각 신경망 동결 (초반 길찾기 학습 집중)
        freeze_layers(model, freeze_modules=['features_extractor'])
    else:
        print("!!! 이전 가중치가 없어 기초부터 학습합니다.")

    # 콜백 리스트
    checkpoint_callback = CheckpointCallback(save_freq=100000 // num_cpu, save_path=LOG_DIR, name_prefix=f'gold_hybrid_{sess_id}')
    unfreeze_callback = UnfreezeCallback(unfreeze_step=2_000_000)
    
    callbacks = CallbackList([checkpoint_callback, unfreeze_callback])

    print(f"\n🚀 하이브리드 학습 시작! (세션: {sess_id})")
    try:
        model.learn(total_timesteps=50_000_000, callback=callbacks)
    except KeyboardInterrupt:
        print("\n중단됨. 현재 모델을 저장합니다.")
    
    model.save(f"{sess_path}/final_gold_hybrid_model")
    env.close()