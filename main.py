...
def make_env(rank, env_conf, ai_conf):
    ...
    def _init():
        configs = env_conf.copy() # 기본 환경 설정 복사
        configs["ai_config"] = ai_conf # LLM/AI 관련 설정 전달
        configs["reward_weights"] = config.REWARD_WEIGHTS # 보상 가중치 전달
    ...
    # return _init
        ...
        if mode == "train":
            configs["instant_win_battles"] = True
            configs["ai_config"]["use_ai_coach"] = False

        elif mode == "play":
            configs["instant_win_battles"] = False
            configs["ai_config"]["use_ai_coach"] = True
