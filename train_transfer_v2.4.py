...
class GameStatsCallback(BaseCallback):
    def __init__(self, verbose=0):
        ...
        self.keys = [
            ...
            "stats_explore", "stats_level_sum", "stats_badges", "stats_deaths", "stats_steps"
        ]
        
        self.name_mapping = {
            # === 0. 종합 ===
            "reward_total":   "0_종합/총점",
            
            # === 1. 보상 (Rewards) ===
            "reward_explore": "1_보상/탐험",
            "reward_new_map": "1_보상/신규맵",
            "reward_battle":  "1_보상/전투돌입",
            "reward_level":   "1_보상/레벨업",
            "reward_exp":     "1_보상/경험치",
            "reward_event":   "1_보상/이벤트",
            "reward_badge":   "1_보상/배지",
            "reward_heal":    "1_보상/회복",
            "reward_dmg":     "1_보상/데미지",
            "reward_stuck":   "1_보상/비비기",
            
            # === 2. 통계 (Stats) ===
            "stats_explore":  "2_통계/방문좌표",
            "stats_level_sum":"2_통계/레벨합",
            "stats_badges":   "2_통계/배지수",
            "stats_deaths":   "2_통계/사망수",
            "stats_steps":    "2_통계/생존스텝"
        }
    ...
    def _on_rollout_end(self) -> None:
        for key in self.keys:
            ...
            if c > 0:
                kor_name = self.name_mapping.get(key, f"기타/{key}")
                self.logger.record(kor_name, self.sums[key] / c)
...
if __name__ == '__main__':
    ...
    if target_weights:
        print(f"이어하기 모드: {target_weights}")
        model = PPO.load(
            target_weights,
            env=env,
            tensorboard_log=log_dir,
